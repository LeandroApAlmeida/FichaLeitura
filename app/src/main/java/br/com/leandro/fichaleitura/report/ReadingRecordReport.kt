...
class ReadingRecordReport(fileName: String): PdfReport(fileName) {
    ...
    override suspend fun print(params: List<Any>) {
        ...
        headerTable.addCell(getCell(getBoldText("FICHA DE LEITURA"), TextAlignment.LEFT))
        ...
        val recordsList = recordDao.getAllRecordsAsc(beginDate.time, endDate.time)
        var totalAmount = 0
        for (record in recordsList) {
            totalAmount++
            val date1 = Date(record.beginDate)
            val date2 = if (record.endDate != null) Date(record.endDate!!) else null
            ...
            if (record.readingCompleted) {
                ...
                dataTable.addCell(Cell().add(getPlainText("---------------", textSize)))
            } else {
                dataTable.addCell(Cell().add(getPlainText("CANCElADA", 6.5f)))
            }
        }
        ...
        if (recordsList.isNotEmpty()) {
            ...
            resumeTable.addCell(Cell().add(getBoldText("TOTAL DE LIVROS NO PERÍODO:", textSize)))
